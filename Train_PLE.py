...
def to_categorical(y, num_classes=None, dtype='float32'):
    ...
    categorical[np.arange(n), y] = 1  # 改为one-hot编码
    output_shape = input_shape + (num_classes,)  # (input_shape, num_classes)
    # 在高维数据的情况下保持输入 y 的原始结构，只在最后增加 one-hot 维度
...
def getTensorDataset(my_x, my_y):
    # 将输入的my_x转换为numpy的float32类型，然后转换为PyTorch的Tensor
    tensor_x = torch.Tensor(my_x.astype(np.float32))
    # 将输入的my_y直接转换为PyTorch的Tensor
    tensor_y = torch.Tensor(my_y)
    # 返回一个TensorDataset对象，其中包含转换后的tensor_x和tensor_y, 便于 DataLoader 进行批量训练
...
def test(loader):
    ...
    with torch.no_grad():
        ...
        for x, y in loader:
            ...
            yhat = model(x)
            y1, y2 = y[:, 0], y[:, 1]  # 任务 1 & 任务 2 的真实标签
            yhat_1, yhat_2 = yhat[0], yhat[1]  # 任务 1 & 任务 2 的预测值
